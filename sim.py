...
Physics concepts modelled or visualised:
...
MAX_P     = 1000
TRAIL_LEN = 30
SF_N      = 250         # background star count
...
def _build_lut(n=512):
    ...
    Represents particle energy/temperature state: hot ejecta → cold inspiral.
    """
...
SF_B   = (_srng.uniform(0,1,SF_N)**2.8*0.28+0.03).astype(np.float32)
SF_RAD = (_srng.uniform(0,1,SF_N)*0.75+0.45).astype(np.float32)
...
LEGEND = [
    (0xFF2200, "Stellar matter / hot ejecta",       "age ≈ 1.0  (fresh)"),
    ...
    (0x2255FF, "Cold gas / ejected particles",       "age ≈ 0.0  (recycled)"),
]
...
def render_dark_bh(gui, i, sx_i, sy_i, r_px, sr_i, bh_pos_i, bh_rad_i,
    ...
    """
    fov_f  = 0.85
    n_d    = N_DISC if quality==2 else (int(N_DISC*0.65) if quality==1 else N_DISC//2)
    ...
    # Chromatic Doppler shift + disc temperature (Planck peak shift)
    # Approaching (doppler_raw > 0): blue-shifted → blue-white
    # Receding (doppler_raw < 0): red-shifted → amber-orange
    temp = np.float32(0.4 + disc_temp_v * 0.6)
    r_f  = np.where(doppler_raw>0, np.clip((0.75-0.4*doppler_raw)*beam*temp,0,1),
                                    np.clip(1.0*beam*temp,0,1))
    g_f  = np.where(doppler_raw>0, np.clip((0.88+0.12*doppler_raw)*beam*temp*0.82,0,1),
                                    np.clip((0.50-0.25*np.abs(doppler_raw))*beam*temp,0,1))
    b_f  = np.where(doppler_raw>0, np.clip((0.82+0.65*doppler_raw)*beam*temp,0,1),
                                    np.clip(0.07*beam*temp,0,1))

    # Radial brightness: inner disc hottest — accretion luminosity ∝ 1/r (Novikov-Thorne)
    radfall = np.clip((1.0 - _r_frac[:n_d])**1.3 * disc_bri[:n_d] * bright, 0.0, 1.0)
    ...
    if np.any(vis):
        pts    = np.stack([sc_nx_l[vis], sc_ny_l[vis]], axis=1).astype(np.float32)
        radii  = (disc_size[:n_d][vis] * (1.0 + radfall[vis]*1.4)).astype(np.float32)
        cols   = c_int[vis].astype(np.uint32)
        gui.circles(pts, radius=radii, color=cols)
...
def main():
    ...
    while gui.running:
        ...
        # ── Particle trails — batch gui.lines (one call total per frame) ────
        ...
        gui.text("PARTICLE  TYPES",(lx0+0.005,ly_top+0.002),font_size=9,color=0x7788BB)
